...
$id=$_POST['hidId'];
$name=$_POST['name'];
$hid=$_POST['hidHeaderId'];
$slug=strtolower($_POST['slug']);
$hidslug=$_POST['hidslug'];
$status=$_POST['sts'];
$date=date('d-m-Y h:i:sa');
if ($slug==$hidslug) {
	$delQry=mysqli_query($con,"update websitepage set name='$name',url='$slug', status='$status',date='$date' where `id`='$id'");
	if($delQry){
	header("location:customerpagecontent.php?msg=pdg&pid=$pid");
	}else{
	header("location:customerpagecontent.php?msg=inff&pid=$pid");
	}
}else{
	if (checkPageSlug($hid,$slug)=='0') {
		$delQry=mysqli_query($con,"update websitepage set name='$name',url='$slug', status='$status',date='$date' where `id`='$id'");
		if($delQry){
		header("location:customerpagecontent.php?msg=pdg&pid=$pid");
		}else{
		header("location:customerpagecontent.php?msg=inff&pid=$pid");
			...
			header("location:customerpagecontent.php?msg=url&pid=$pid");
	}
}
...
if($delQry){
header("location:customerpagecontent.php?msg=dls&pid=$pid");
}else{
header("location:customerpagecontent.php?msg=dlf&pid=$pid");
}
}
...
switch($msg){
case 'ins':
$msg='<strong>Success!</strong> Section has been added Successfully !!';
$class='success';
break;
case 'pdg':
$msg='<strong>Success!</strong> Page Updated Successfully !!';
$class='success';
break;
case 'inf':
$msg='Section Data not updated Successfully !!';
$class='danger';
break;
case 'ups':
$msg='<strong>Success!</strong> Section updated Successfully !!';
$class='success';
break;
case 'url':
$msg=' slug allready exists !!';
$class='danger';
break;
...
<head>
<meta charset="utf-8" />
<title>customer Page Content </title>
...
<a href="customeraddmore.php?pid=<?php echo base64_encode($page['id']) ?>"	<button type="button" class="btn btn-success btn-md waves-effect waves-light">Add Section</button></a>
</div>
</div>
</div>
...
<i class="fas fa-arrows-alt"></i>
	...
	<div>
		...
		<a href="customereditcontent.php?cid=<?php echo base64_encode($fetch['id']) ?>" title="Message"><button type="button" class="btn btn-success btn-md waves-effect waves-light"><i class="bx bx-edit"></i> Edit</button></a>
		<a href="#del" class=delete id="<?php echo ($fetch['id']) ?>"  ><button style="margin-left:10px" data-bs-toggle="modal" data-bs-target="#del" class="btn btn-danger btn-md " type="button"><i class="bx bx-trash" ></i> Delete</button></a></td>
...
<input  type="hidden" id="hidHeaderId" name="hidHeaderId" class="form-control" value="<?php echo $page['header_id'] ?>"/>
<input onKeyUp="fillPrintableName()"  type="text" id="name" name="name" class="form-control" value="<?php echo $page['name'] ?>"/>
</div>
<div  class="mb-3 col-lg-12">
<label for="slug">Slug</label>
<input style="text-transform:lowercase"  type="text" id="slug" name="slug" class="form-control" value="<?php echo $page['url'] ?>"/>
<input style="text-transform:lowercase"  type="hidden" id="hidslug" name="hidslug" class="form-control" value="<?php echo $page['url'] ?>"/>
...
<button type="submit" name="update" class="btn btn-success btn-md w-md">Update </button> 
<button onclick="goBack()" type="button" name="cancel" class="btn btn-outline-success btn-md w-md">Cancel </button>
</div>
</form>
...
var b=$("#hidid").val();
//alert(b);
var x="<a  href='customerpagecontent.php?did="+a+"&pid="+b+"'><button class='btn btn-success'>Yes delete it!</button></a>";
$('#delbtn').html(x);
})
</script>
</body>
